...
## Деплой
`vercel.json` → `outputDirectory: "."`, без build-шага. `.vercelignore` исключает `.Codex`, `references`, `*.pdf` и неиспользуемые тяжёлые экспорты (`img/turntable.*`, часть `images/*`, не referenced из index.html).
